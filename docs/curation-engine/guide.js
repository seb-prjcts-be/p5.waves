  ...
  registerSketch('guide-closability', function(p) {
    const SAW_CIRCLE = [
      { name: 'saw up',   period: 50, color: [255, 200, 80] },
      { name: 'saw down', period: 50, color: [255, 120, 160] },
      { name: 'ramp',     period: 50, color: [140, 220, 255] }
    ];
    const SPIKY = [
    ...
    function drawProof(choice, A, B, radius, phaseX, phaseY, drawnTo, prog) {
      ...
      for (let i = 0; i <= SEGMENTS; i++) {
        ...
        const x = Waves.wave(A * theta + phaseX * period, { wave: choice.name, amplitude: 1 });
        const y = Waves.wave(B * theta + phaseY * period + period * 0.25, { wave: choice.name, amplitude: 1 });
        xs[i] = x; ys[i] = y;
        if (x < xMin) xMin = x; if (x > xMax) xMax = x;
        if (y < yMin) yMin = y; if (y > yMax) yMax = y;
      }
      // Re-center the shape in its panel so asymmetric waves (saw, ramp)
      // don't sit hard against one edge.
      ...
      p.endShape();
      const homeR = 11 + p.sin(prog * p.TWO_PI) * 1.4;
      p.noStroke();
      p.fill(255); p.circle(xs[0], ys[0], homeR);
      p.fill(18);  p.circle(xs[0], ys[0], homeR - 5);
      p.fill(255); p.circle(xs[drawnTo], ys[drawnTo], 7);
    }

    p.setup = function() {
      const size = canvasSize('guide-closability', 0.5, 260, 420);
      p.createCanvas(size.width, size.height).parent('guide-closability');
      p.pixelDensity(1);
    ...
    p.draw = function() {
      p.background(12);
      const cycleId  = p.floor(p.millis() / CYCLE_MS);
      ...
      const drawnTo  = p.floor(prog * SEGMENTS);

      const halfW  = p.width / 2;
      const radius = p.min(halfW, p.height) * 0.34;

      p.stroke(35);
      p.strokeWeight(1);
      p.line(halfW, 20, halfW, p.height - 20);

      p.push();
      p.translate(halfW * 0.5, p.height / 2);
      drawProof(SAW_CIRCLE[sawIdx], 1, 1, radius, phaseX, phaseY, drawnTo, prog);
      p.pop();

      p.push();
      p.translate(halfW * 1.5, p.height / 2);
      drawProof(SPIKY[spikyIdx], 3, 5, radius, phaseX, phaseY, drawnTo, prog);
      p.pop();

      p.noStroke();
      p.fill(230);
      p.textSize(12);
      p.text('Saw Circle', 16, 22);
      p.text('Spiky Lissajous', halfW + 16, 22);
      p.fill(140);
      p.textSize(10);
      p.text(SAW_CIRCLE[sawIdx].name + '  /  A:B = 1:1', 16, 38);
      p.text(SPIKY[spikyIdx].name + '  /  A:B = 3:5', halfW + 16, 38);
      p.fill(120);
      p.text('click either half to cycle  /  each pen returns to its white dot',
             16, p.height - 14);
    };
    ...
    p.mousePressed = function() {
      ...
      if (p.mouseX < p.width / 2) {
        sawIdx = (sawIdx + 1) % SAW_CIRCLE.length;
      } else {
        spikyIdx = (spikyIdx + 1) % SPIKY.length;
      }
    };
